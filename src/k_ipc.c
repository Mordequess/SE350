...
void* k_receive_message(int *sender_id) {
	...
	__disable_irq();
	while (!m_any_messages_from_sender(current_process, *sender_id)){//} current_process msg_queue is empty ) {
		get_pcb_pointer_from_process_id(current_process)->m_state = BLOCKED_ON_RECEIVE ;
		release_processor() ;
	}
	m = m_dequeue(current_process, *sender_id);
	__enable_irq();
...
message* m_dequeue(int destination_id, int sender_id) {
	...
	if (!m_is_empty(destination_id)) {
		while (element->sender_id != sender_id) {
			element = element->mp_next;
		}
		m_remove_queue_node(destination_id, element);
		return element;
	}
